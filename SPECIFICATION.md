...
Если запись указывает на локальный драйвер, вызывается асинхронный метод driver.send_command(address, args). Драйвер преобразует команду в проприетарный формат и отправляет целевому устройству.
