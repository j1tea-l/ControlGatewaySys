...
class TelemetryBridge(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data: bytes, addr: tuple[str, int]) -> None:
        ...
        if summary is None:
            ...
            return
        logger.info("TELEMETRY RX kind=%s address=%s from=%s:%s bytes=%s ts=%.6f", summary["kind"], summary["address"], addr[0], addr[1], len(data), time.time())
    ...
    async def handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            if summary is None:
                ...
                return
            logger.info("TELEMETRY RX kind=%s address=%s from=%s bytes=%s ts=%.6f", summary["kind"], summary["address"], peer, len(data), time.time())
            for target in self.targets:
                _send_udp(target, data)
...
async def start_telemetry_bridge(listen_ip: str, listen_port: int, targets: list[TelemetryForwardTarget], transport: str = "udp"):
    loop = asyncio.get_running_loop()
    if transport == "tcp":
        server = await asyncio.start_server(TelemetryTCPServer(targets).handle, listen_ip, listen_port)
        logger.info("TELEMETRY BRIDGE started transport=tcp ip=%s port=%s targets=%s", listen_ip, listen_port, [t.name for t in targets])
        return server
    udp_transport, _ = await loop.create_datagram_endpoint(lambda: TelemetryBridge(targets), local_addr=(listen_ip, listen_port))
    logger.info("TELEMETRY BRIDGE started transport=udp ip=%s port=%s targets=%s", listen_ip, listen_port, [t.name for t in targets])
